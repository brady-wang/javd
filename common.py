...
class Test:
    ...
    def parse_list(self, content):
        ...
        ul = html.xpath('//*[@id="makers"]//div[@class="box"]//a')
        if len(ul) > 0:
            for li in ul:
                item = {}
                href = li.xpath('./@href')
                title = li.xpath('./strong/text()')
                number = li.xpath('./span/text()')

                if len(href) > 0 and len(title) > 0 and len(number) > 0:
    ...
    try:
        ...
        for k in LIST:
            ...
            if mark_type == "zimu_wuma":
                ...
                file_name = "./cili/zimu/youma/" + k['title'] + ".txt"
            elif mark_type == "all_wuma":
                file_name = "./cili/all/wuma/" + k['title'] + ".txt"
            else:
